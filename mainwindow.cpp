...
void MainWindow::packetArrival(long ts_sec,long ts_usec,unsigned long long total_len,QByteArray data){
    unsigned long long timestamp = static_cast<unsigned long long>(ts_sec)*1000000 + ts_usec;
    if(!base_time.has_value())
        base_time = timestamp;
    int id = table->rowCount();
    table->insertRow(id);
    table->setItem(id,0,new QTableWidgetItem(QString::number(id),QTableWidgetItem::Type));
    table->setItem(id,1,
                   new QTableWidgetItem(QString::number((timestamp - base_time.value())*1.0 / 1000000),
                                        QTableWidgetItem::Type)
                   );
    auto packet = reinterpret_cast<const PacketInterpreter::EthernetPacket*>(data.data());
    table->setItem(id,2,new QTableWidgetItem(packet->getDisplaySource(),QTableWidgetItem::Type));
    table->setItem(id,3,new QTableWidgetItem(packet->getDisplayDestination(),QTableWidgetItem::Type));
    table->setItem(id,4,new QTableWidgetItem(packet->get_protocols().back(),QTableWidgetItem::Type));
    table->setItem(id,5,new QTableWidgetItem(QString::number(total_len),QTableWidgetItem::Type));
    table->setItem(id,6,new QTableWidgetItem(packet->get_comment(),QTableWidgetItem::Type));
...
void MainWindow::startCapture(QString friendly_name,QString adapter){
    ...
    table->setRowCount(0);
    table->setHorizontalHeaderLabels({tr("No."),tr("Time"),tr("Source"),tr("Destination"),tr("Protocol"),tr("Length"),tr("Info")});
    ...
    connect(table->selectionModel(),&QItemSelectionModel::selectionChanged,table,[=](){
        auto rows = table->selectionModel()->selectedRows();
        if(rows.isEmpty())return;
        const auto& ref_data = std::get<2>(this->packets[rows[0].row()]);
        auto details_data = reinterpret_cast<const PacketInterpreter::EthernetPacket*>(ref_data.data())->get_details();
        ...
        std::function<void(QLayout* layout, QWidget* parent, const std::wstring& name,const PacketInterpreter::my_ptree& ptree)> insert_ptree =
                [details,hex_view,&insert_ptree](QLayout* layout,QWidget* parent, const std::wstring& name,const PacketInterpreter::my_ptree& ptree)
        {
            if(ptree.empty()){
                auto *label = new QPushButton(QString::fromStdWString(name)+":"+std::get<0>(ptree.data()), parent);
                ...
                connect(label,&QPushButton::clicked,label,[range_left,range_len,hex_view](){
                    hex_view->setSelected(range_left,range_len);
                });
            }else{
                ui::Section* section = new ui::Section(QString::fromStdWString(name),ui::Section::DEFAULT_DURATION,parent);
                section->resize(parent->size());
                QVBoxLayout *section_layout = new QVBoxLayout;
                for(const auto& child:ptree){
                    insert_ptree(section_layout,section,child.first,child.second);
                }

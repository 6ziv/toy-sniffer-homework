...
constexpr std::pair<std::string_view,std::string_view> protocols[]={
    ...
    {"VINES","VINES"},
    {"TTP/IPTM","Transaction Transport Protocol/Internet Protocol Traffic Manager"},
...
inline uint8_t translate_protocol_id(uint8_t id){
    if(id<144)return id;
    if(id<253)return 144;
    return id -108;
}
QString get_protocol_name(uint8_t id){
    uint8_t realid = translate_protocol_id(id);
    if(protocols[realid].first.empty())return QString::number(id);
    return QString::fromLocal8Bit(protocols[realid].first.data(),protocols[realid].first.size());
}
QString get_protocol_description(uint8_t id){
    uint8_t realid = translate_protocol_id(id);
    if(protocols[realid].second.empty())return QString("[no description]");
    return QString::fromLocal8Bit(protocols[realid].second.data(),protocols[realid].second.size());
}
...
QString get_tcp_option_name(uint8_t id){
    if(id<31)return QString::fromLocal8Bit(tcp_option_tags[id].data(),tcp_option_tags[id].size());
    if(id==34)return QString::fromLocal8Bit(tcp_option_tags[31].data(),tcp_option_tags[31].size());
    if(id==69)return QString::fromLocal8Bit(tcp_option_tags[32].data(),tcp_option_tags[32].size());
    if(id==253)return QString::fromLocal8Bit(tcp_option_tags[33].data(),tcp_option_tags[33].size());
    if(id==254)return QString::fromLocal8Bit(tcp_option_tags[34].data(),tcp_option_tags[34].size());
    return "Reserved";
}

...
unsigned long long timeval_to_ulonglong(const timeval* ts){
    if(ts->tv_sec>=0)return static_cast<unsigned long long>(ts->tv_sec)*1000000 + ts->tv_usec;
    return (static_cast<unsigned long long>(
                std::numeric_limits<unsigned long>().max()
                )+1)*1000000 + ts->tv_usec;
}
struct UData{
    QObject* obj;
  ...
  const u_char *pkt_data)
{
    ...
    unsigned long long bps =
            reinterpret_cast<const unsigned long long*>(pkt_data)[1]
            * 1000000

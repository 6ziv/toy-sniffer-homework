...
{}
void dispatcher_handler_sniff(u_char *state,
  ...
  const u_char *pkt_data)
{
    ...
    QMetaObject::invokeMethod(
                ...
                "packetArrival",
                Qt::QueuedConnection,
                Q_ARG(long,header->ts.tv_sec),
                Q_ARG(long,header->ts.tv_usec),
                Q_ARG(unsigned long long,header->len),
...
void SniffThread::run()
{
    ...
    if(pcap==nullptr){
        LOG(ERROR)<<"Can not listen on adapter "<<std::quoted(adapter_name.c_str())<<"\n";
    ...
    pcap_activate(pcap);
    pcap_loop(pcap,-1,dispatcher_handler_sniff,reinterpret_cast<uchar*>(this->parent()));
    pcap_close(pcap);
    //pcap_next()
}
